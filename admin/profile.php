...
try {
    ...
    $_SESSION['error'] = 'Failed to load admin data.';
    $admin = [];
}
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare("UPDATE users SET 
                                ...
                                WHERE id = ?");
            $stmt->execute([
                $name, $email, $phone, $address, $admin_id
            ]);
            ...
            // Refresh admin data to reflect changes
            $stmt = $db->prepare("SELECT * FROM users WHERE id = ?");
            $stmt->execute([$admin_id]);
            $admin = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
    ...
    <div class="col-md-4 mb-4">
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="fas fa-id-card me-2"></i>Admin Information</h5>
            </div>
            <div class="card-body text-center">
                ...
                <div class="text-start">
                    ...
                    <p><strong>Joined:</strong> <?php echo date('d M Y', strtotime($admin['created_at'])); ?></p>
                </div>
            </div>
        </div>
    </div>
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="email" name="email" id="email" class="form-control" value="<?php echo $admin['email']; ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="tel" name="phone" id="phone" class="form-control" value="<?php echo $admin['phone']; ?>">
                        </div>
                        <div class="col-md-6">
